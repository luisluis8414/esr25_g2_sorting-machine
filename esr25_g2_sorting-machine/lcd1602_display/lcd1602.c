...
void write8BitI2CtoDisplay(uint8_t data)
{
    ...
    char buf[3];
    // Prepare all three states at once to minimize I2C overhead
    buf[0] = data;
    buf[1] = data | EN;
    buf[2] = data;

    // Send all states in one I2C transaction
    I2C_write(SLAVE_ADDRESS_LCD, buf, 3);
    timer_sleep_ms(1); // Only one delay needed
}
...
void write4BitI2CtoDisplay(uint8_t data, bool cmd)
{
    ...
    uint8_t local_data;
    char buf[6]; // Space for both nibbles (3 bytes each)
    int idx = 0;
    ...
    // Send all in one I2C transaction
    I2C_write(SLAVE_ADDRESS_LCD, buf, 6);
    timer_sleep_ms(1); // Only one delay needed
}
